...
from services.send_api_companies_data import SendApiCompaniesData
...
class CompaniesDataExtract:
    def __init__(self, logger: Logger):
        ...
        self.__sendToApi = SendApiCompaniesData()
    ...
    async def processAsync(self):
        try:
            ...
            for companie in companies:
                try:
                    sql = readSql(os.path.join(folderSrc, "sqls"), "companies_data.sql", {"codi_emp": str(companie["codiEmp"])})
                    ...
                    self.__logger.info(f'Save success companies_data {companie["codiEmp"]} - {companie["nameEmp"]} - {companie["federalRegistration"]}')
